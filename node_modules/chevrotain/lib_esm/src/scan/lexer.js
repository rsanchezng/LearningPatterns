...
export function analyzeTokenTypes(tokenTypes, options) {
    ...
    var onlyRelevantTypes = reject(tokenTypes, function (currType) {
        return currType[PATTERN] === Lexer.NA;
    });
    var hasCustom = false;
    var allTransformedPatterns = map(onlyRelevantTypes, function (currType) {
    ...
    var patternIdxToType = map(onlyRelevantTypes, function (currType) { return currType.tokenTypeIdx; });
    var patternIdxToGroup = map(onlyRelevantTypes, function (clazz) {
    ...
    var patternIdxToLongerAltIdx = map(onlyRelevantTypes, function (clazz) {
    ...
    var patternIdxToPushMode = map(onlyRelevantTypes, function (clazz) { return clazz.PUSH_MODE; });
    var patternIdxToPopMode = map(onlyRelevantTypes, function (clazz) {
        return has(clazz, "POP_MODE");
    });
    var lineTerminatorCharCodes = getCharCodes(options.lineTerminatorCharacters);
    var patternIdxToCanLineTerminator = map(onlyRelevantTypes, function (tokType) { return false; });
    ...
    var patternIdxToIsCustom = map(onlyRelevantTypes, isCustomPattern);
    var patternIdxToShort = map(allTransformedPatterns, isShortPattern);
    var emptyGroups = reduce(onlyRelevantTypes, function (acc, clazz) {
        var groupName = clazz.GROUP;
    ...
    var patternIdxToConfig = map(allTransformedPatterns, function (x, idx) {
        return {
    ...
    if (!options.safeMode) {
        charCodeToPatternIdxToConfig = reduce(onlyRelevantTypes, function (result, currTokType, idx) {
            if (typeof currTokType.PATTERN === "string") {
                ...
                if (options.ensureOptimizations) {
                    PRINT_ERROR("" + failedOptimizationPrefixMsg +
                        ("\tTokenType: <" + tokenName(currTokType) + "> is using a custom token pattern without providing <start_chars_hint> parameter.\n") +
                        "\tThis will disable the lexer's first char optimizations.\n" +
...
export function findModesThatDoNotExist(tokenTypes, validModes) {
    ...
    var errors = map(invalidModes, function (clazz) {
        var msg = "Token Type: ->" + tokenName(clazz) + "<- static 'PUSH_MODE' value cannot refer to a Lexer Mode ->" + clazz.PUSH_MODE + "<-" + "which does not exist";
        return {
            message: msg,
    ...
    forEach(tokenTypes, function (tokType, testIdx) {
        forEach(canBeTested, function (_a) {
            ...
            if (testIdx < idx && testTokenType(str, tokType.PATTERN)) {
                var msg = "Token: ->" + tokenName(tokenType) + "<- can never be matched.\n" +
                    ("Because it appears AFTER the Token Type ->" + tokenName(tokType) + "<-") +
