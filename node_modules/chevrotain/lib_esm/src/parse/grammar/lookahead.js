...
export function buildLookaheadFuncForOr(occurrence, ruleGrammar, k, hasPredicates, dynamicTokensEnabled, laFuncBuilder) {
...
export function lookAheadSequenceFromAlternatives(altsDefs, k) {
    function getOtherPaths(pathsAndSuffixes, filterIdx) {
        return reduce(pathsAndSuffixes, function (result, currPathsAndSuffixes, currIdx) {
            if (currIdx !== filterIdx) {
                var currPartialPaths = map(currPathsAndSuffixes, function (singlePathAndSuffix) { return singlePathAndSuffix.partialPath; });
    ...
    function isUniquePrefix(arr, item) {
        return (find(arr, function (currOtherPath) {
            return every(item, function (currPathTok, idx) {
...
export function getLookaheadPathsForOr(occurrence, ruleGrammar, k) {
...
export function containsPath(alternative, path) {
    var found = find(alternative, function (otherPath) {
        return (path.length === otherPath.length &&
            every(path, function (targetItem, idx) {
